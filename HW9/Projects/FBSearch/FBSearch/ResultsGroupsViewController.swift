//
//  ResultsGroupsViewController.swift
//  FBSearch
//
...
class ResultsGroupsViewController: UIViewController,UITableViewDelegate,UITableViewDataSource{
    ...
    func getDataFromServer(){
       ...
       // var stringArray = keyWordTextFromUser.components(separatedBy: " ")
        ...
        Alamofire.request("http://sample-env.3ksm2sdv9u.us-west-2.elasticbeanstalk.com/index.php?key=\(formatedKey)&type=group&det=false&tab=true").responseJSON { response in
            
            let jsonObject = JSON(response.result.value!)
            
            
            
            self.jsonFromSearch = jsonObject["group"]
            // print(self.jsonFromSearch)
            
            self.nextButton.isHidden = true
            self.previousButton.isHidden = true
            
            if let next = jsonObject["group"]["paging"]["next"].string{
                self.nextButton.isHidden = false
                self.nextLink = next
            }
            if let prev = jsonObject["group"]["paging"]["previous"].string{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "GroupsCell", for: indexPath) as! ResultsCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let name = jsonFromSearch ["data"][indexPath.row]["name"].string,
            ...
            typeForDetailsSearch = "group"
            
           profilePicForDetilsSearch = jsonFromSearch["data"][indexPath.row]["picture"]["data"]["url"].string!            
            
            performSegue(withIdentifier: "GroupResultsDetail", sender: nil)
