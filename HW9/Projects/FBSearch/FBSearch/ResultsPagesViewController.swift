//
//  ResultsPagesViewController.swift
//  FBSearch
//
...
class ResultsPagesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
...
//        }
        ...
        Alamofire.request("http://sample-env.3ksm2sdv9u.us-west-2.elasticbeanstalk.com/index.php?key=\(formatedKey)&type=pages&det=false&tab=true").responseJSON { response in
            
            let jsonObject = JSON(response.result.value)
            
           
            
            self.jsonFromSearch = jsonObject["pages"]
           // print(self.jsonFromSearch)
            
            self.nextButton.isHidden = true
            self.previousButton.isHidden = true
            
            if let next = jsonObject["pages"]["paging"]["next"].string{
                self.nextButton.isHidden = false
                self.nextLink = next
            }
            if let prev = jsonObject["pages"]["paging"]["previous"].string{
    ...
    */
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "PagesCell", for: indexPath) as! ResultsCell
